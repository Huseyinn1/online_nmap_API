...
def scan_by_type(req: ScanTypeRequest):
    ...
    scan_types = {
        "discovery": lambda ip: run_generic(ip, "-sn"),
        "os": lambda ip: run_generic(ip, "-O"),
        "light": lambda ip: run_generic(ip, "-sV -T4 -p 21,22,23,25,53,80,110,111,135,139,143,443,445,993,995,1723,3306,3389,5900,8080"),
        "medium": lambda ip: run_generic(ip, "-sV -T4 -p 20-25,53,80,110-111,135-139,143,443,445,993,995,1723,3306,3389,5900,8080,8443"),
        "heavy": lambda ip: run_generic(ip, "-sV -T4 -p-"),
        "vuln-light": lambda ip: run_generic(ip, "-sV -T4 --script vuln -p 21,22,23,25,53,80,110,111,135,139,143,443,445,993,995,1723,3306,3389,5900,8080"),
        "vuln-medium": lambda ip: run_generic(ip, "-sV -T4 --script vuln,auth,default -p 20-25,53,80,110-111,135-139,143,443,445,993,995,1723,3306,3389,5900,8080,8443"),
        "vuln-heavy": lambda ip: run_generic(ip, "-sV -T4 --script vuln,auth,default,version -p-"),
        "web": lambda ip: run_generic(ip, "-sV -T4 --script http-* -p 80,443,8080,8443,8888,9000"),
        "db": lambda ip: run_generic(ip, "-sV -T4 --script db-* -p 1433,3306,5432,6379,27017"),
        "full": lambda ip: run_generic(ip, "-sS -sV -O --script vuln,dns-brute,http-enum --traceroute -T4"),
        "security": lambda ip: run_generic(ip, "-sV -T4 --script auth,vuln,default -p 21,22,23,25,53,80,110,111,135,139,143,443,445,993,995,1723,3306,3389,5900,8080"),
        "service-version": lambda ip: run_generic(ip, "-sV -T4 --version-intensity 9 -p-"),
        "ssl": lambda ip: run_generic(ip, "-sV -T4 --script ssl-* -p 443,465,993,995,8443"),
        "dns": lambda ip: run_generic(ip, "-sV -T4 --script dns-* -p 53"),
        "smtp": lambda ip: run_generic(ip, "-sV -T4 --script smtp-* -p 25,465,587"),
        "ftp": lambda ip: run_generic(ip, "-sV -T4 --script ftp-* -p 21,2121"),
        "smb": lambda ip: run_generic(ip, "-sV -T4 --script smb-* -p 139,445"),
        "quick": lambda ip: run_generic(ip, "-sV -T4 --top-ports 100"),
        "full-security": lambda ip: run_generic(ip, "-sV -T4 --script auth,vuln,default,version,discovery -p-")
    }
...
def scan_and_export_by_type(req: ScanTypeRequest):
    ...
    # PDF oluştur
    scan_id = str(uuid4())
    export_to_pdf(scan_results, f"exports/{scan_id}.pdf")
    
    return {
        "scan_id": scan_id,
        "message": "PDF oluşturuldu",
        "file_path": f"exports/{scan_id}.pdf",
